...
export const InputField: React.FC<InputField> = ({ label, value, placeholder, makeBigger, onChange, error, type }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (type === 'number') {
      ...
      if (!isWithinRange(currentValue, parseFloat(event.currentTarget.min), parseFloat(event.currentTarget.max))) {
  ...
  <div>
    ...
    <input
      ...
      className={`bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-text focus:outline-none focus:ring-1 focus:ring-british-green-0 focus:border-british-green-0 sm:text-sm
                ${error? 'rounded-md ring-inset-2 ring-2 ring-red-400' : ''} ${type == 'number'? 'appearance-none' : ''}`}
    />)}
  </div>
);
};
