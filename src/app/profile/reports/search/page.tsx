...
export const ReportsSearchPage: React.FC<ReportsSearchProps> = async ({searchParams: {query}}) => {
    ...
    const GetDateFilter = async (formData: FormData) => {
        ...
        if (searchQuery)
        redirect("/profile/reports/search?query=" + searchQuery)

        const currentDate = new Date();
        const twoWeeksAgo = await subDays(currentDate, +searchQuery!);
            ...
            <div>
                <form action={GetDateFilter} className="flex flex-wrap items-center h-full max-w-md mx-auto mt-8">
                    ...
                    <select name="searchQuery" className="mt-2 flex-2 bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-6 py-2 text-left cursor-default focus:outline-none focus:ring-british-green-4 focus:border-british-green-4 sm:text-sm ">
                        ...
                        <option value="14" className="cursor-default select-none relative py-2 pl-3 pr-9">14 days</option>
                        <option value="31" className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">31 days</option>
                    </select>
                    <button 
                    type="submit"
